...
export default function Courses() {
    ...
    fetch(`http://localhost:4000/v1/category`)
      .then((res) => {
        if(res.ok){
          ...
          return res.text().then(txt => new Error(txt))
        }
      }).catch(err => {
        console.log(err)
      })
  ...
  const addNewCourse = (event) => {
    ...
    if (courseCategory === '-1' || !formState.isFormValid) {
      swal({
        title : "لطفا مقدار های هر اینپوت را بدرستی وارد کنید!",
        icon : 'warning'
      })
    } else {
      fetch(`http://localhost:4000/v1/courses`, {
            ...
            <div class="col-6">
              <div class="number input">
                ...
                <select onChange={selectCategory}>
                  <option value="-1">لطفا دستبندی مورد نظر خود را انتخاب کنید.</option>
